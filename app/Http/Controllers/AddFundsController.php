  ...
  protected function sendCoinpayments()
  {
        ...
        try {
          ...
          $amountFixed = number_format($this->request->amount + ($this->request->amount * $fee / 100) + $cents + $taxes, 2, '.', '');
            ...
            $preference->back_urls = [
              'success' => route('mercadopadoProcess',
              [
                'userId' => auth()->user()->id,
    ...
    public function sendCoinbase()
    {
      ...
      $amountFixed = number_format($this->request->amount + ($this->request->amount * $fee / 100) + $cents + $taxes, 2, '.', '');
        ...
        $checkout = $httpClient->request('POST', 'https://api.commerce.coinbase.com/charges', [
                'headers' => [
          ...
          if ($request_json !== false && !empty($request_json)) {
              ...
              if ($hmac == $recived_hmac) {
                  if (isset($response['payment_status']) && isset($response['payment_id'])) {
                    ...
                    if ($payload['payment_status'] === 'finished' && isset($data)) {
    ...
    public function sendMpesa()
    {
        try {
          ...
          $mpesaTransaction->taxes = $taxes?? null;
          ...
          return response()->json([
            "success" => true,
            "status" => 'pending',
            'status_info' => __('Payment requested successfully. Kindly check your phone.')
          ]);
