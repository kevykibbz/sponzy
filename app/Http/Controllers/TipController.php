...
class TipController extends Controller
{
    ...
    try {
      ...
      STK::init(
        array(
            ...
            'callback_url'     => route('mpesaTipIpn'),
        )
      );
      ...
      $mpesaTransaction->isLive = $this->request->isLive? 1: 0;
  ...
  public function mpesaTipIpn(Request $request){
    $response = isset($request['Body']) ? $request['Body'] : [];
      ...
      if ($response != null && $response['stkCallback']['MerchantRequestID'] != null) {
          ...
          if($mpesaTransaction && $mpesaTransaction->status == 0){
            ...
            $user = User::find($mpesaTransaction->media_id);

            $AuthUser = User::find($mpesaTransaction->user_id);
            ...
            if ($user->email_new_tip == 'yes') {
              $this->notifyEmailNewTip($user, $AuthUser->username, $amount);
            }

            // Send Notification
            Notifications::send($user->id, $AuthUser->id, '5', $AuthUser->id);

            //============== Check if the tip is sent by message
            if ($mpesaTransaction->isMessage === 1) {
              $this->isMessageTip($user->id, $AuthUser->id, $amount);
            }
